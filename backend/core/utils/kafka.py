...
def is_kafka_available(host: str, port: int, timeout: float = 2.0) -> bool:
    try:
        ...
        logger.critical(f"[Kafka Check] Connection failed: {e}")
        return False
            ...
            async for message in self.consumer:
                try:
                    ...
                    # if payload.get("action") == "create":
                        ...
                        #     )
                        #     logger.info(f"[Kafka Consumer] Indexed product {product_data['id']} into Elasticsearch.")
                except Exception as inner_ex:
                    logger.error(f"[Kafka Consumer] Error processing message: {inner_ex}")
        except Exception as e:
            logger.critical(f"[Kafka Consumer] Fatal error: {e}")
...
class KafkaProducer:
    ...
    async def send(self, message: dict):
        ...
        try:
            ...
            logger.info(f"[Kafka Producer] Message sent.-> {message}")
        except Exception as e:
            logger.error(f"[Kafka Producer] Failed to send message: {e}")
            raise
...
async def send_kafka_message(kafka_producer: KafkaProducer, message: dict):
    try:
        ...
        logger.info("[Kafka Helper] Message sent successfully.")
    except Exception as e:
        logger.error(f"[Kafka Helper] Error sending message: {e}")
